...
<nav aria-label="Page navigation example">
  <ul class="pagination">
    ...
    <?php
      if($prev_page > 1) {
        echo '<li class="page-item"><a class="page-link" href="foodtap.php?a='.$_GET["a"].'&b='.$_GET["b"].'&page='.$prev_page.'">Prev</a></li>';
      }
      ...
      for($i = $start_page; $i <= $end_page; $i++) {
        if($i == $page) {
          ...
          echo '<li class="page-item"><a class="page-link" href="foodtap.php?a='.$_GET["a"].'&b='.$_GET["b"].'&page='.$i.'">'.$i.'</a></li>';
        }
      }

      $next_page = $end_page + 1;
      if($next_page <= $total_page) {
        echo '<li class="page-item"><a class="page-link" href="foodtap.php?a='.$_GET["a"].'&b='.$_GET["b"].'&page='.$next_page.'">Next</a></li>';
      }

      if($page < $total_page) {
        echo '<li class="page-item"><a class="page-link" href="foodtap.php?a='.$_GET["a"].'&b='.$_GET["b"].'&page='.$total_page.'">Last</a></li>';
      }
    ?>
    </ul>
</nav>